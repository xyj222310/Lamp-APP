...
/**
 * Created by yjtse on 2016/11/24.
 * UDP发送命令处理类
 * 用于生成发送字节流
 */
...
public class UdpOrderUtil {
    ...
    public byte[] getSendOrderFrameBytes(byte type, byte[] datas) {
        int length = getFrameLength(type);
        ...
        if (length < 4) {
            ...
            result[length - 2] = result[1];  //0xaa 0x80 0x01 0x02 0x83 0x55
        } else { // length > 4 的情况
            ...
            result[length - 2] = GetCheckSum(checksum);
        }
        result[length - 1] = end;
        return result;
    }
